...
\begin{frame}{CARE -- Mục tiêu huấn luyện $\beta_\psi$}
  ...
  \vspace{0.4em}
  \begin{block}{Diễn giải gradient}
    \footnotesize $\partial \mathcal{L}_\beta / \partial \log \beta_\psi(s)$ tỉ lệ với covariance trong batch giữa $I^+(s)$ và $\hat{A}^E(s)$:
    \begin{itemize}\footnotesize
      \item \textbf{Cov dương} (intrinsic cao đi đôi với advantage dương) $\Rightarrow$ \textit{tăng} $\beta$.
      \item \textbf{Cov âm} (intrinsic cao đi đôi với advantage âm)  $\Rightarrow$ \textit{giảm} $\beta$.
      \item \textbf{Cov $\approx 0$} (cold-start, sparse) $\Rightarrow$ regularizer chiếm ưu thế, $\beta \to \beta_0$.
    \end{itemize}
  \end{block}
\end{frame}

% =============================================================
% Slide 8 - Tai sao dung advantage thay return
% =============================================================
\begin{frame}{Tại sao dùng \textit{advantage} thay vì \textit{return}?}
  Một lựa chọn thiết kế quan trọng của CARE: tín hiệu giám sát cho Beta Network là \textbf{GAE advantage} $\hat{A}^E$, không phải \textit{discounted return} $G^E$.

  \vspace{0.3em}
  \begin{columns}[T,onlytextwidth]
    \column{0.5\textwidth}
    \textbf{Vấn đề của $G^E$:}
    \begin{itemize}\small
      \item Variance bị chi phối bởi \textit{vị trí trong episode}.
      \item Trạng thái đầu episode "thấy" gần như toàn bộ $\sum \gamma^k r_k$.
      \item Trạng thái cuối episode "thấy" hầu như 0.
      \item $\Rightarrow$ tín hiệu giả: state đầu luôn có return cao hơn, không phản ánh đóng góp riêng của state.
    \end{itemize}

    \column{0.48\textwidth}
    \textbf{Lợi ích của $\hat{A}^E = G^E - V(s)$:}
    \begin{itemize}\small
      \item Trừ baseline $V(s)$ \textbf{loại bỏ confound vị trí episode}.
      \item Tín hiệu là "vượt kỳ vọng baseline bao nhiêu" -- credit per-state sạch hơn.
      \item Phù hợp đúng với trực giác "khuếch đại nội tại nơi chính sách đang vượt baseline".
    \end{itemize}
  \end{columns}

  \vspace{0.3em}
  \begin{block}{Trách nhiệm cài đặt}
    \footnotesize Beta Network nhận \textbf{advantage thô (un-normalized)} để giữ thông tin độ lớn tín hiệu. PPO policy update vẫn dùng advantage đã chuẩn hoá như chuẩn mực.
  \end{block}
\end{frame}
...
\begin{frame}{Thiết kế thực nghiệm}
  \begin{columns}[T,onlytextwidth]
    ...
    \end{itemize}
    \vspace{0.3em}
    \textbf{Mỗi seed: $10^6$ bước môi trường, $\sim$2--4 giờ trên T4.}
  \end{columns}
  ...
  \begin{block}{Bốn chỉ số đánh giá}
    ...
    \begin{tabular}{@{}p{2.6cm}p{10.0cm}@{}}
      Episode return    & Mean reward 10 episode cuối, trung bình 5 seeds. \\
      Sample efficiency & Số bước đầu tiên đạt $0.7 \times$ best asymptotic. \\
      Learning stability & Std-dev chéo seed của reward curve. \\
...
% =============================================================
% Slide 10 - Ket qua 1: CARE-COUNT & CARE-RIDE
% =============================================================
\begin{frame}{Kết quả: CARE-COUNT \& CARE-RIDE -- khớp hoặc vượt}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{itemize}\footnotesize
      ...
      \item Variance thấp nhất trên mọi env mà CARE-COUNT hội tụ.
    \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{Kết quả: $\beta_\psi(s)$ thật sự phụ thuộc trạng thái}
  ...
  \begin{enumerate}\scriptsize
    ...
    \item \textbf{Extrinsic giữa training} (KeyCorridor, RedBlueDoors, UnlockPickup): $\beta_\psi$ \textit{neo tại $\beta_0$} cold-start, chỉ tách khi advantage có magnitude.
    \item \textbf{Phân phối cuối $\beta_\psi$ trải $\sim$1 bậc giá trị} qua các state -- bằng chứng học được hàm \textit{thật sự phụ thuộc trạng thái}, không collapse về một hằng.
  \end{enumerate}
\end{frame}
...
\begin{frame}{Phân tích thất bại: vì sao CARE-ICM kém?}
  ...
  \vspace{0.3em}
  \begin{block}{Bài học}
    \footnotesize CARE \textbf{không phải} thay thế cho việc chọn module nội tại tốt. Đó là một bộ nhân phụ thuộc trạng thái mà chất lượng phụ thuộc vào \textit{độ tương quan} giữa tín hiệu nội tại thô và extrinsic-success path.
  \end{block}
\end{frame}
...
\begin{frame}{Kết luận \& Giới hạn}
  ...
  \begin{itemize}\small
    ...
    \item \textbf{Cold-start trên môi trường cực thưa} (Empty-16x16): advantage gần như 0 ở mọi state, gradient correlation ~ noise, $\beta_\psi$ rớt về $\beta_0$ -- CARE \textit{thoái biến về fixed-$\beta$}.
...
\begin{frame}{Hướng phát triển \& Câu hỏi}
  ...
  \begin{center}
    ...
    {\Large\itshape Sẵn sàng nhận câu hỏi.}\\[0.8em]

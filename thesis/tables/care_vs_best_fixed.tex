\begin{table}[H]
\centering
\caption{CARE versus the best post-hoc fixed-$\beta$ value, per (environment, module). Gap is $r_\text{CARE} - r_\text{best-FB}$ in raw reward; positive means CARE is better. Best-$\beta$ is the value of $\beta$ that achieves $r_\text{best-FB}$ on that environment.}

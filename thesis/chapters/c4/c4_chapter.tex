...
The global intrinsic strength is fixed at $\alpha = 10^{-3}$ for every intrinsic reward module so that any difference between fixed-$\beta$ and CARE conditions can be attributed to the scaling factor itself rather than to a change in overall intrinsic magnitude. In the CARE conditions, the Beta Network is realized as a two-layer encoder of dimension $256$ followed by a $128$-unit head producing $\log \beta_\psi$, which is clipped so that $\beta_\psi(s_t) \in [\beta_\text{min}, \beta_\text{max}] = [0.1, 2.0]$. The learning-progress objective in Equation~\eqref{eq:beta_loss} uses regularization weight $\lambda_\text{reg} = 10^{-3}$, reference value $\beta_0 = 1.0$, gradient clipping at $L_2$ norm $1.0$, and $L_2$ weight decay of $10^{-6}$ on the Beta Network parameters. The Beta Network is updated with Adam at learning rate $5 \times 10^{-4}$. In the fixed-$\beta$ conditions, the Beta Network is replaced by a constant scalar $\beta \in \{0.005, 0.05, 0.5, 1.0, 2.0\}$ and the learning-progress update is disabled; the rest of the pipeline (intrinsic generation, standardization, and the shaped reward formula in Equation~\eqref{eq:combined_reward}) is unchanged.
...
$F_{m,\beta}$ & PPO + module $m$ with constant $\beta$, $m \in \{\text{Count}, \text{ICM}, \text{RIDE}\}$, $\beta \in \{0.005,\, 0.05,\, 0.5,\, 1.0,\, 2.0\}$ & Fixed-coefficient sweep for each module. The five values span four orders of magnitude, covering both ``intrinsic almost off'' and ``intrinsic comparable to extrinsic'' regimes. \\ \hline
...
\begin{itemize}
    \item \textbf{$B_0$ vs $F_{m,\beta}$} measures whether intrinsic motivation in module $m$ at coefficient $\beta$ helps or hurts on top of the extrinsic-only PPO baseline. Sweeping $\beta$ over four orders of magnitude exposes how sensitive each module is to manual tuning.
...
\textit{Analysis of how sharply performance varies across the swept range $\beta \in \{0.005, \dots, 2.0\}$ for each module, motivating the need for adaptive scaling: modules whose performance varies strongly with $\beta$ are the ones where CARE is expected to help most.}

...
This chapter describes the experimental setup used to evaluate the proposed methodology. It first presents the benchmark environments and the computing infrastructure, then specifies the experimental configuration, the baselines, and the performance evaluation metrics. The empirical evaluation across all three intrinsic reward families (count-based, ICM, and RND) is the subject of Section~\ref{sec:empirical_results}. Settings reported here are the canonical configuration shared by every reported run; the underlying hyperparameter values are tabulated in Appendix~\ref{app:implementation}.
...
$B_\text{RND}$ & PPO + RND with $\beta \equiv 1$ & RND exploration with fixed unit coefficient. \\ \hline
...
$C_\text{RND}$ & PPO + RND + CARE & RND bonus with learned $\beta_\psi(s_t)$. \\ \hline
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{$C_\text{Count}$ vs $C_\text{ICM}$ vs $C_\text{RND}$} reveals how the same CARE layer interacts with structurally different intrinsic generators, addressing the research question of module-agnosticism stated in Chapter~1.
\end{itemize}
